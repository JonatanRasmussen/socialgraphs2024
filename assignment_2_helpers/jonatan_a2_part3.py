...
labmt_df = pd.read_csv('a2_part3_data.txt', sep='\t', na_values='--')
print(labmt_df.head())  #LabMT wordlist dataset
with open('country_music_data.pkl', 'rb') as f:
    data = pickle.load(f)  # Access the components
artist_texts = data['performer_text']  # Wiki content for sentiment analysis
performer_contents = data['performer_contents']  # Link structure
G = data['graph_directed']  # Directed network
G_undirected = data['graph_undirected']  # Undirected network
content_size = data['content_size']  # Page lengths
